...
def home():
    movies = (
        model.Movie.query.order_by(model.Movie.rating.desc()).limit(5).all()
    )  # Gets the 5 best rated movies
    movies.append(movies[0])  # Needed for the slider animation
    all_movies = model.Movie.query.order_by(
        model.Movie.id.desc()
    ).all()  # All movies by newest
...
def search():
    ...
    if order_criteria == "newest":
        all_movies = (
            model.Movie.query.filter(model.Movie.name.startswith(str))
        ...
        all_movies = (
            model.Movie.query.filter(model.Movie.name.startswith(str))
            .order_by(
                model.Movie.duration_hours.desc(), model.Movie.duration_min.desc()
            )
            .all()
        )
...
def post_reservation(movie_id=0):
    ...
    projection = (
        model.Projection.query.filter_by(id=projection_id)
    ...
    if remaining_seats < seats:
        abort(
            403,
            f"There are not enough seats, try another projection (only: {remaining_seats} seats available)",
        )
...
def projection(projection_id=1):
    ...
    if user.admin:
        ...
        img = analytics.seats_available(
            {"Projection: " + str(projection.id): (free_seats, reserved_seats)}
        )
        return render_template(
            "main/projection.html",
            projection=projection,
            user=user,
            analytics_plot=img.decode("utf-8"),
        )
...
def post_projection(projection_id=1):
    ...
    if form_type == "cancel_reservation":
        ...
        reservation = model.Reservation.query.filter_by(
            id=reservation_id
        ).first_or_404()
        ...
        db.session.commit()
        return redirect(
            url_for("main.user_template", user_id=flask_login.current_user.id)
        )
...
def user_template():
    ...
    if not user.admin:
        # Reservations
        reservations = (
            model.Reservation.query.filter_by(user_id=user.id)
    ...
    movie_analytics_plot = analytics.movie_analytics(
        {movie.name: movie.rating for movie in movies}
    )

    # Programmed projecitons
    projections = model.Projection.query.filter(
        model.Projection.date > datetime.datetime.now()
    ).all()
    ...
    for projection in projections:
        ...
        for reservation in projection.reservations:
            ...
            reserved_seats += reservation.seats
            data[
                str(projection.movie.name)
                + ", room "
                + str(projection.screen.id)
                + ", "
                + str(projection.date)[:-9]
            ] = (free_seats, reserved_seats)

    img = analytics.seats_available(data, figsize=(6, int(len(data) * 0.9)))

    return render_template(
        "main/admin_template.html",
        movies=movies,
        screens=screens,
        user=user,
...
def post_user():
    ...
    # Admin can not cancel reservations or give feedback
        ...
        if form_type == "new_projection":
            ...
            existent_projection = (
                model.Projection.query.filter_by(screen_id=screen_id)
                .filter(
                    model.Projection.date
                    >= date - datetime.timedelta(hours=-2, minutes=-30)
                )
                .filter(
                    model.Projection.date
                    <= date + datetime.timedelta(hours=-2, minutes=-30)
            ...
            projection = model.Projection(
                screen_id=screen_id, movie_id=movie_id, date=date
            )
            db.session.add(projection)
            db.session.commit()
        elif form_type == "cancel":
            projection_id = int(request.form.get("projection_id"))
            projection = model.Projection.query.filter_by(
                id=projection_id
            ).first_or_404()
            cancellation_emails(projection.reservations)
            model.Projection.query.filter_by(id=projection_id).delete()
